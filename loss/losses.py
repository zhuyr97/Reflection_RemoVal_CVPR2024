...
import loss.pytorch_colors as colors
...
class Vgg19(torch.nn.Module):
    def __init__(self, requires_grad=False):
        ...
        self.vgg.load_state_dict(torch.load('/gdata1/zhuyr/Deref/vgg19-dcbb9e9d.pth'))
        self.vgg.eval()
...
class HSVLoss(nn.Module):
    ...
    def forward(self, predict, target):
        ...
        predict_hsv = colors.rgb_to_hsv(predict.detach())#
        target_hsv = colors.rgb_to_hsv(target.detach())#
        predict_hsv = torch.unsqueeze(predict_hsv[:,0,:,:], 1)
        target_hsv = torch.unsqueeze(target_hsv[:,0,:,:], 1)
        # print('target_hsv: ', target_hsv.shape)

        temp = torch.ones_like(predict_hsv)
        temp1 = torch.zeros_like(predict_hsv)

        diff = torch.abs(predict_hsv - target_hsv)
        mask = torch.where(diff > temp*0.5, temp, temp1)
        mask_ = torch.where(temp * 0.5 > diff, temp, temp1)

        loss = torch.mean(torch.abs(diff*mask_)) \
               + torch.mean(torch.abs(diff*mask - 0.5*mask))
        return loss
